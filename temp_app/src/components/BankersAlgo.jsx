...
const BankersAlgo = () => {
    ...
    const runSafetyAlgorithm = () => {
        ...
        let logA=[];
        let count = 0;
        let failLogged = Array(pCount).fill(false);
        while (count < pCount) {
            let found = false;
                ...
                if (!finish[p]) {
                    ...
                    for (let j = 0; j < rCount; j++) {
                        if (need[p][j] > work[j]) {
                            canAllocate = false;
                            if (!failLogged[p]) {
                                let needVal = need[p][j];     // ← Need ของ resource j
                                let availVal = work[j]; 
                                logA.push(`P${p} Resource ${String.fromCharCode(65+j)} → Need: ${needVal}, Available: ${availVal}, Finish: F `);
                                failLogged[p] = true; 
                            }
                            break;
                        }
                    ...
                    if (canAllocate) {
                        for (let j = 0; j < rCount; j++) {
                            ...
                            let needVal = need[p][j];     // ← Need ของ resource j
                            let availVal = work[j]; 
                            logA.push(`P${p} Resource ${String.fromCharCode(65+j)} → Need: ${needVal}, Available: ${availVal}, Finish: T`);
                    ...
                    {result.error ? (
                        ...
                        <>
                            <h3 style={{ color: '#10b981', marginBottom: '0.5rem' }}>System is in a SAFE state </h3>
                                ...
                                <div style={{ marginTop: '2rem', padding: '1rem', background: 'rgba(0,0,0,0.05)', borderRadius: '0.5rem' }}>
                                    <h3 style={{ marginBottom: '1rem' }}>Process</h3>
                                    
                                    <pre>{logA.join('\n')}</pre>
                                    <p>Final Available Resources:</p>
                                        <ul>
                                            {logW.map((val, i) => (
                                                <li key={i}>
                                                    Resource {String.fromCharCode(65 + i)}: {val}
                                                </li>
                                            ))}
                                        </ul>
                                </div>
